...
% anaylzed. NOTE: the only string normalization performed on the string is
% converting upper to lower case, does not check for spaces etc. 
prompt3 = "Which cave data would you like to use Natural Bridge Cavern(NBC) or Harrie Wood Cave(HWC)? Enter NBC or HWC?";
...
prompt = "How many cluster groups would you like to make (2/3/4 cluster groups)?";
no_of_cgroups = input(prompt);
prompt2 = "Pick sampling frequncy by typing daily, weekly or monthly: ";
frequency = input(prompt2,'s');
frequency = lower(frequency);
% similarly to before the only text normalization used is removing upper
% case
